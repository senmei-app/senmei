...
- **Real-ESRGAN TorchScript conversion** — `scripts/convert_realesrgan.py` converts the official `RealESRGAN_x4plus.pth` state dict into a loadable TorchScript `.pt` (classic RRDBNet, no `pixel_unshuffle`, two **2×** nearest upsamples = 4×). Verified via the ignored `torch_loads_realesrgan` test (loads in `TorchEngine`, 64→256). The `.pt` itself is **not committed** (`models/*.pt` ignored); `metadata.json` sets `realesrgan-x4plus.loadable: true` and drops `download_url`/`sha256` (they pointed at the unloadable `.pth`). Requires `torch` + `libtorch` to build/run with the `torch` feature.
