...
/// rendering via `ImageData` (no `<img>`/PNG round-trip). Both transports
/// deliver raw bytes — Tauri over IPC (`ArrayBuffer` channel), HTTP as an
/// `ArrayBuffer` body with `x-frame-width`/`x-frame-height` headers. No base64.
...
export interface Backend {
  ...
  /// Small JPEG thumbnail of a file (data URL) + its probe — one call, so the
  /// library tile's size/codec line needs no separate `probeVideo`.
  thumbnail(input: string): Promise<ThumbnailResult>;
