...
fn passthrough_pause_resume() {
    ...
    while frames.load(std::sync::atomic::Ordering::Acquire) == 0
        && std::time::Instant::now() < deadline
    {
        std::thread::sleep(std::time::Duration::from_millis(10));
    }
    pause.store(true, std::sync::atomic::Ordering::Release);
    // Let in-flight frames drain, then verify the counter stabilises while
    // paused. Poll for up to 2 s instead of a single snapshot — avoids
    // flakes on fast runners where a 300 ms window can still see tail frames.
    std::thread::sleep(std::time::Duration::from_millis(500));
    let during = frames.load(std::sync::atomic::Ordering::Acquire);
    let verify_deadline = std::time::Instant::now() + std::time::Duration::from_secs(2);
    let mut during2 = during;
    while std::time::Instant::now() < verify_deadline {
        std::thread::sleep(std::time::Duration::from_millis(200));
        during2 = frames.load(std::sync::atomic::Ordering::Acquire);
        if during2 == during {
            break;
        }
    }
    ...
    handle.join().unwrap();
    let total = frames.load(std::sync::atomic::Ordering::Acquire);
