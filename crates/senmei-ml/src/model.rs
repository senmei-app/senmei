...
mod tests {
    ...
    fn registry_loads_example_metadata() {
        ...
        assert_eq!(registry.models()[0].id, "shuffle-cugan");
        assert!(registry.models()[0].loadable);
        let weights = registry.models()[0].weights.as_deref().unwrap();
        assert_eq!(weights[0], "sudo_shuffle_cugan_9.584.969.pth.f16.bpk");
        assert_eq!(registry.models()[1].id, "real-cugan-x2");
        assert!(matches!(registry.models()[1].kind, ModelKind::Upscale));
        assert_eq!(registry.models()[1].scale, 2);
