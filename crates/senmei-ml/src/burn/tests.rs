//! End-to-end burn-engine tests (Vulkan; most `#[ignore]`d — need models/GPU).
...
fn tensor_data_f16_to_vec_is_available() {
    ...
    let data = burn::tensor::TensorData::new(vec![0.5f32, 1.0, 0.0, 0.25], [4]).convert::<f16>();
...
fn scunet_tiled_ghosts_at_tile_seams() {
    let bpk =
        std::path::Path::new("/home/mzach/.local/share/senmei/models/scunet_color_15.pth.f16.bpk");
    if !bpk.exists() {
        eprintln!("missing bpk, skipping");
        return;
    }
